...
const getResellerProfileByUserId = async (userId) => {
...
const updateResellerProfile = async (resellerId, data) => {
...
const getDashboardStats = async (resellerId) => {
  const reseller = await Reseller.findByPk(resellerId);
  if (!reseller) {
    throw new Error('Reseller not found');
  }

  const earnedStats = await ResellerEarning.findAll({
  ...
  const pendingStats = await ResellerEarning.findAll({
