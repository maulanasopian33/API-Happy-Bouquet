...
const { creditFund, debitFund } = require('./fundService');
...
const getAllOrders = async (page = 1, limit = 20, customerId) => {
...
const confirmPayment = async (orderId) => {
  ...
  try {
    ...
    if (templates.length > 0) {
      const costItems = templates.map((tpl) => {
...
const payWorkerFees = async (orderId) => {
  const t = await sequelize.transaction();
  try {
    const order = await Order.findByPk(orderId, { transaction: t });
    if (!order) throw new Error('Order tidak ditemukan');
    ...
    const totalToPay = unpaidLabor.reduce((sum, item) => sum + Number(item.amount), 0);
