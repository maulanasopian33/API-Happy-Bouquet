...
const getResellerOrders = async (resellerId, page = 1, limit = 20) => {
...
const getResellerOrderById = async (orderId, resellerId) => {
...
const createResellerOrder = async (resellerUserId, data) => {
...
const uploadPaymentProof = async (orderId, resellerId, fileUrl) => {
  const order = await Order.findOne({
    where: { id: orderId, reseller_id: resellerId },
  });
  if (!order) {
