...
describe('Cookie Auth (Dual-mode) & CSRF', () => {
  ...
  it('harus mengembalikan token di body DAN mengeset cookie hb_token saat login', async () => {
    ...
    const cookie = cookies.find((c: string) => c.startsWith('hb_token='));
  ...
  it('logout harus menghapus cookie', async () => {
    ...
    expect(res.status).toBe(200);
    const cleared: string[] = Array.isArray(res.headers['set-cookie'])
      ? (res.headers['set-cookie'] as unknown as string[])
      : [res.headers['set-cookie'] as string];
    const clearCookie = cleared.find((c: string) => c.startsWith('hb_token='));
